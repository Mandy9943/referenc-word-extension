...
async function callGeminiFormatter(references, apiKeyOverride) {
...
app.post("/api/gemini-format", async (req, res) => {
  ...
  try {
    const headerKey = (req.headers["x-workflow-gemini-key"] || "").trim();
    const text = await callGeminiFormatter(references, headerKey || undefined);
    res.json({ text });
  } catch (error) {
